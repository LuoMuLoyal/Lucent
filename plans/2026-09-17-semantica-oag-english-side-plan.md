...
| **S5**              | 1 天   | **已落地（2026-09-19）**：Lucent 新增 `reason_over_ontology` 工具，四个注册点 + policy 门控 + sidecar 客户端；NL→Cypher 按 §3.5 切分（Lucent 生成、sidecar 校验执行）+ 报错回喂重试（§8.1 风险 2）    | 工具返回实际执行的 Cypher + 来源等级（`sourceTier`）的 envelope；**PROV-O 引用未接**（sidecar 未暴露 provenance 端点） |
...
真实模型）。两处与原始验收口径的偏差记明：① **PROV-O 引用未接** —— envelope 带的是实际执行
的 Cypher + `sourceTier`（`drugbank_structured`）作为可复核路径；② **sidecar 尚无
Dockerfile / 镜像** —— compose 服务定义已就位但容器化部署不可用，dev 走本机 `uvicorn`。
仍未做的是 **S0（英文侧评测集）与 S3（§4.1 词表 → 手写 SHACL/OWL）**。
