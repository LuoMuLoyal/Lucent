...
| **S5**              | 1 天   | **已落地（2026-09-19）**：Lucent 新增 `reason_over_ontology` 工具，四个注册点 + policy 门控 + sidecar 客户端；NL→Cypher 按 §3.5 切分（Lucent 生成、sidecar 校验执行）+ 报错回喂重试（§8.1 风险 2）；同日补齐**边缘溯源到端到端引用**（边上 `prov` → sidecar `/provenance` → envelope `citations` → SSE → 来源条） | 工具返回实际执行的 Cypher + 来源等级（`sourceTier`）+ **逐条 PROV-O 引用**（`citations`，可解到来源表 / 行键 / 原文 / 哈希链位置）的 envelope；sidecar 侧 `verify_chain` 220,011 条无断链 |
...
**S5 落地实情（2026-09-19）**：工具链路已通并真机验证（真实 AGE 图 3278 节点 / 211,630 边 +
真实模型）。**PROV-O 引用已接**：图上每条边带 `prov` id，sidecar `/provenance` 把它解成来源表 /
行键 / 原文 / 哈希链位置（220,011 条审计记录，`verify_chain` 无断链），envelope 带 `citations`、
SSE 带 `citations` + `executedQuery`，来源条可点击复制 id 复核。仍与原始口径有偏差的是：
**sidecar 尚无 Dockerfile / 镜像** —— compose 服务定义已就位但容器化部署不可用，dev 走本机
`uvicorn`。仍未做的是 **S0（英文侧评测集）与 S3（§4.1 词表 → 手写 SHACL/OWL）**。
另记一条实测教训（已进 prompt）：图上名称按 DrugBank 原样**大写**存储，`{name: $x}` 精确
匹配在大小写不一致时**静默返回 0 行**，会被如实说成"DrugBank 没有这条断言"——生成侧强制
`toLower()` 匹配后才返回真实行。
