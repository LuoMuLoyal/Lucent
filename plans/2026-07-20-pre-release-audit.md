...
发版前需关注的 **3 个潜在隐患** 如下（原阻断级问题 A1/A2/A3 已全部修复，P2 项 B1/B4 已修复）。
...
- `@Cron` 数据保留清理已实现（过期会话、已读通知 30 天、过期反馈抑制）
...
1. 账户删除流程增加：匿名化导出 → 级联硬删除（保留期后）

**优先级**：P3（清理管道已完成，账户删除级联为后续迭代）
...
- **B2**：环境数据接入真实天气 API
- **B3**：多实例限流迁移到 Redis
- **B4**：账户删除级联清理（匿名化导出 → 硬删除保留期，清理管道已完成）
- **B5**：`nonDeleted` 查询迁移
