...
Sparse Record Semantics 已完成跨仓库收口。Review Experience Task 1/2/3 已落地 event review 读模型、四段 section 子服务与三个 review endpoint + OpenAPI 导出；Task 4（Luminous 客户端，`dart run tool/bootstrap_generated_sources.dart` 后接入 domain/repository/provider）尚未完成，仍作为后续边界保留。`buildCurrent` 双重读取收敛为低优先级遗留项，`doseLogSources` capped 判定与 red flag 用户级限制保持文档化限制。Report legacy scalar fallback 在 Review 迁移前继续保留；Water ml/coverage、服药槽位和睡眠 episode 的后端语义已完成。Proactive Suggestion Runtime 保留为已完成的服务端主动重算基础。
