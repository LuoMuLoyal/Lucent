...
Sparse Record Semantics 已完成跨仓库收口，Review Experience 进入 Task 2：event review 读模型骨架（DTO、`EventReviewService` 三种只读入口、ownership 门面只读扩展）已落地，四段 section 计算、controller/endpoint 与 Luminous 客户端尚未完成，仍作为后续边界保留。Report legacy scalar fallback 在 Review 迁移前继续保留；Water ml/coverage、服药槽位和睡眠 episode 的后端语义已完成。Proactive Suggestion Runtime 保留为已完成的服务端主动重算基础。
