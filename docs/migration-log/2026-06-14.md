...
- Switched server deployment to a split layout: tracked app repo at `/opt/lucent/app`, server-local runtime files at `/opt/lucent/server`.
- Updated compose and deploy workflow so the server now `git pull --ff-only`, then reads env/certs/Nginx runtime files from `/opt/lucent/server`.
