...
- Classified 8 auth TODOs: 2 resolved (OAuth config boot validation via OnModuleInit), 6 remain
  with owner/reason tags.
- Rewrote remaining TODOs with blocked conditions: 2FA (product decision), session management
  (feature scope), OAuth-only password (security review), security notifications (notification
  infra), Apple/Google providers (platform credentials).
