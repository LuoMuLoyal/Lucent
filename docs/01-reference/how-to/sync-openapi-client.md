...
- 如在 Luminous 侧操作，确认 `../Lucent/docs/openapi.json` 路径可达
...
```bash
# 检查路径数和 schema 数是否合理
node -e "const s=require('./docs/openapi.json'); console.log('paths:', Object.keys(s.paths).length, 'schemas:', Object.keys(s.components.schemas).length)"
```
...
- [[../../../../Lucent/docs/01-reference/how-to/sync-openapi-client]] （本文）
- `Luminous/docs/02-reference/OpenApi_Client.md` — 生成物边界与使用规则
- `Lucent/docs/01-reference/contracts/README.md` — API 合同边界
