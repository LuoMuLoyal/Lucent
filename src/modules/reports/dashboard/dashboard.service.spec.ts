...
import type { ReportsComputationService } from './computation.service.js';
import type { ReportsContextService } from './context.service.js';
...
describe('ReportsService', () => {
  ...
  it('recomputes from the DB when the cache get fails (fail-open)', async () => {
    const contextService = {
      build: vi.fn().mockResolvedValue({
        range: REPORT_RANGE_LAST_7_DAYS,
        startDate: new Date('2026-06-06T00:00:00.000Z'),
    ...
    // A cache read failure must not turn into a 5xx: fall back to the DB.
    expect(contextService.build).toHaveBeenCalledWith('u1', {
      range: REPORT_RANGE_LAST_7_DAYS,
    });
