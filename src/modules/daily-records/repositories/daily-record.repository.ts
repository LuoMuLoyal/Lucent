...
export abstract class DailyRecordRepositoryPort {
   ...
   * `$transaction`. Prisma nested transactions silently degrade to independent
   * connections, losing atomicity. Side effects (queue enqueue, cache
   * invalidation, etc.) must also be moved outside the callback — only DB
   * writes that need atomicity belong here.
   */
