...
export function buildAssistantSystemPrompt(
  ...
  return [
    ...
    'Daily records of kind `meal` carry `mealAnalysisStatus` (`analyzed`, `analyzing`, or `analysis_failed`) with tags like `meal_estimate:analyzed`, plus `mealShortDescription` (the most important finding) and `mealTopFoods` (recognized dish names). Meal energy is an interval, not a single value: never present it as precise. When a meal analysis failed, treat it as unavailable evidence rather than silent omission, and never treat an `analyzing` record as a finished result.',
