...
describe('ProfileCollectorService', () => {
  ...
  it('returns a profile_completeness signal with all fields missing when profile is null', async () => {
    (prisma.userAllergy.count as jest.Mock).mockResolvedValue(0);
    (prisma.userCondition.count as jest.Mock).mockResolvedValue(0);
  ...
  it('returns isComplete=true when all required fields are present', async () => {
    (prisma.userAllergy.count as jest.Mock).mockResolvedValue(2);
    (prisma.userCondition.count as jest.Mock).mockResolvedValue(1);
